"""
Business: OAuth-вход администратора сообщества VK через КЛАССИЧЕСКИЙ протокол
(oauth.vk.com, Standalone-приложение) — получает токен ПОЛЬЗОВАТЕЛЯ с правами
groups+photos, который backend vk-market-sync использует ТОЛЬКО для загрузки
фото в товары (photos.getMarketUploadServer не работает с токеном сообщества
и не работает с токенами нового VK ID — проверено, обе схемы возвращают
ошибки VK 27/28/1051 «method is unavailable»).
Сами товары (add/edit/delete) управляются отдельным токеном сообщества
(VK_COMMUNITY_TOKEN), эта функция его не трогает.
Args: event с httpMethod GET, queryStringParameters {action: status, code, state, error}
...
# должен быть прописан в настройках Standalone-приложения VK (Redirect URI).
...
def handler(event, context):
    ...
    # Классический OAuth возвращает наш ?action=... как есть (в отличие от VK ID),
    # но редирект от VK при ошибке/успехе тоже различаем по факту наличия code/error.
    ...
    if is_vk_callback:
        ...
        if not code:
            return _err(400, 'Нет code в ответе VK')
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # state сверяем, только если он был сохранён (защита от CSRF)
                if vk_state:
                    cur.execute(
                        f"SELECT 1 FROM {SCHEMA}.vk_oauth_pending WHERE state = %s AND created_at > NOW() - INTERVAL '{PENDING_TTL_MINUTES} minutes'",
                        (vk_state,)
                    )
                    if not cur.fetchone():
                ...
                # Шаг 2: обмен authorization_code на access_token классическим способом.
                token_q = urllib.parse.urlencode({
                    'client_id': app_id,
                ...
                try:
                    with urllib.request.urlopen(f'{VK_OAUTH_TOKEN}?{token_q}', timeout=15) as r:
                        resp = json.loads(r.read().decode())
                ...
                cur.execute(
                    ...
                    (int(group_id) if group_id else 0, vk_user_id, access_token, None, None, expires_at, 'groups,photos')
                )
                conn.commit()
        finally:
            conn.close()
    ...
    # Шаг 1 (действие по умолчанию): генерируем state, сохраняем во временную
    # таблицу (проверка CSRF на шаге 2), отправляем администратора на VK.
    if not app_id:
