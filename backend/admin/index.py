    ...
    if action == 'marketing_stats':
        ...
        cur.execute(f"""
            SELECT
                ...
                (SELECT COUNT(*) FROM {SCHEMA}.listings WHERE status='active' AND (is_visible IS NULL OR is_visible = TRUE)) AS active_listings,
...
def _listings(cur, conn, method, rid, event, user):
    ...
    if method == 'POST':
        ...
        sql = (
            ...
            f"{_bool(body.get('export_domclick'))}, "
            ...
            f"{_int_or_null(body.get('avito_ad_id'))}, "
    ...
    if method == 'PUT' and rid:
        ...
        diff_cols = [
            ...
            'use_watermark', 'export_yandex', 'export_avito', 'export_cian', 'export_other', 'export_youla', 'export_domclick',
        ...
        for f, length in [('title', 255), ('description', 5000), ('ai_notes', 2000), ('category', 50), ('deal', 20),
        ...
        for f in ('price', 'price_per_m2', 'area', 'payback', 'profit', 'floor', 'total_floors',
                  'building_year', 'subway_distance', 'prepay_months', 'passenger_lifts', 'cargo_lifts',
                  'avito_ad_id'):
            if f in body:
        ...
        for f in ('use_watermark', 'export_yandex', 'export_avito', 'export_cian', 'export_other', 'export_youla', 'export_domclick'):
            if f in body:
        ...
        if any(k in body for k in ('export_yandex', 'export_avito', 'export_cian', 'export_other', 'export_youla', 'export_domclick',
...
XML_FEED_FORMATS = ('yandex', 'avito', 'cian', 'youla', 'domclick', 'other', 'market', 'market_vk')
...
def _listings_bulk(cur, conn, event, user):
    ...
    if op == 'archive':
        ...
        allowed = {'yandex': 'export_yandex', 'avito': 'export_avito', 'cian': 'export_cian', 'other': 'export_other', 'youla': 'export_youla', 'domclick': 'export_domclick'}
        if platform == 'all':
