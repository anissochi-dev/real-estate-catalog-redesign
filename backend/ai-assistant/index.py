...
SYSTEM_PROMPTS = {
    ...
    'agent': (
        ...
        '- КРИТИЧНО: НИКОГДА не предлагай действие если у тебя нет конкретных id для его параметров. Если id нет — сначала выполни аудит/поиск (низкий риск), чтобы получить id.\n'
        ...
        '- broken_data.items содержит объекты с полем id — передавай их ids в fix_data_quality: {"issue_type":"wrong_price","ids":[id1,id2,...]}\n'
        '- duplicates.items содержит дубли — передавай ids в fix_data_quality: {"issue_type":"duplicate","ids":[...]}\n'
        '- old_unprocessed_leads.items содержит лиды с полем id — для каждого лида: close_lead или reply_lead с {"id": lead_id}. Предлагай только один конкретный лид с наименьшим id, не все сразу.\n'
        '- НИКОГДА не предлагай archive_listing, fix_data_quality, reply_lead, close_lead без конкретного id в params.\n'
        '- Если items пустой или count=0 — НЕ предлагай соответствующее действие вообще.'
    ),
    'security': (
