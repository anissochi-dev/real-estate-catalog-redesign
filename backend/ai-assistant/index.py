...
SYSTEM_PROMPTS = {
    ...
    'title': (
        ...
        'или «Продаю», «Продам» (если сделка — продажа). Никаких других глагольных форм '
        '(«Сдаётся», «Продаётся», «Сдаём», «Продаём», «Сдадим» и т.п.) использовать ЗАПРЕЩЕНО ни в каком виде.\n'
        '2. ДЛИНА заголовка — СТРОГО от 60 до 70 символов включительно (считай символы, включая пробелы и знаки). '
        'Заголовок короче 60 символов — БРАК, категорически недопустим, это самая частая твоя ошибка — '
        'исправляй её осознанно. Заголовок должен заканчиваться ПОЛНЫМ словом, без обрыва на середине.\n'
        '3. ЧТОБЫ НАБРАТЬ НУЖНУЮ ДЛИНУ, обязательно включи в заголовок НЕ МЕНЕЕ ЧЕТЫРЁХ смысловых блоков из списка: '
        ...
        '7. Без кавычек, без markdown, без точки в конце. Верни ТОЛЬКО текст заголовка, одной строкой.\n'
        'ПРИМЕРЫ ПРАВИЛЬНОЙ ДЛИНЫ (60-70 символов, посчитано точно): '
        ...
        'Перед тем как вернуть ответ, мысленно посчитай длину своего заголовка в символах — '
        'если меньше 60, добавь ещё одну характеристику; если больше 70, убери деталь целиком словом.'
    ),
    'reply_lead': (
