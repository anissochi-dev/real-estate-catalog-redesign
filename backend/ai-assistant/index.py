...
SYSTEM_PROMPTS = {
    'describe': (
        ...
        'ЖЁСТКАЯ СТРУКТУРА (соблюдай неукоснительно, каждый раздел начинай ТОЧНО с указанного заголовка на отдельной строке):\n\n'
        'От собственника! Без комиссий и %!\n'
        'Первая строка описания — всегда именно эта фраза, без изменений.\n\n'
        'Общие параметры и назначение\n'
        'Тип объекта, текущее состояние, под какие виды деятельности идеально подходит (с аргументацией).\n\n'
        'Локация и район\n'
        'Адрес, описание района: престижность, транспортная доступность, трафик.\n'
        'Инфраструктура: жилые массивы, бизнес-центры, торговые центры, образовательные учреждения.\n'
        'Тренды района: новые стройки, планы развития, рост населения и его влияние на стоимость.\n\n'
        'Характеристики объекта\n'
        'Площадь, этаж, планировка, высота потолков.\n'
        'Входная группа: отдельный вход, витринное остекление, место под вывеску.\n'
        'Парковка.\n'
        'Состояние отделки: если "под чистовую" — свобода дизайна; если готовая — старт "завтра".\n\n'
        'Коммуникации\n'
        'Электричество: мощность, стабильность. Водоснабжение и канализация. Отопление, вентиляция. Связь.\n'
        'ВАЖНО: если данных о коммуникациях нет в карточке — раздел «Коммуникации» НЕ включать вообще (ни заголовок, ни текст, ни пометки об отсутствии данных).\n\n'
        'Финансовые перспективы и доходность\n'
        'Если есть арендный доход — детализируй. Расчётная доходность в % годовых и срок окупаемости.\n'
        'Драйверы увеличения дохода.\n\n'
        'Условия и юридическая чистота\n'
        'Документы готовы к сделке, объект свободен от обременений. Возможные бонусы: рассрочка, помощь в оформлении и т.д.\n\n'
        'ВАЖНЫЕ ПРАВИЛА:\n'
        '- Заголовки разделов пиши ТОЧНО как указано выше, на отдельной строке, без двоеточий, без markdown, без звёздочек * и без символа #.\n'
        '- ЗАПРЕЩЕНО использовать ** вокруг заголовков или любого текста — никакого markdown форматирования.\n'
        '- Если данных для раздела нет — раздел пропускай ПОЛНОСТЬЮ: ни заголовок, ни текст, ни пометки "(данные отсутствуют)", "(нет данных)" и подобные.\n'
        '- Если блока характеристик нет в карточке — не придумывай, найди в интернете, если не найдено — не включай.\n'
        '- НИКОГДА не указывай стоимость/цену объекта в описании.\n'
        '- Если в данных есть строка "Особые пожелания к описанию" — обязательно учти их при составлении текста: это прямая инструкция от пользователя и она имеет приоритет над остальными правилами стиля (кроме жёсткой структуры и запрета цены/markdown).\n'
        '- Объём — строго не более 3000 символов.'
    ),
    'title': (
        'Ты — опытный коммерческий брокер с 15-летним стажем. '
        'Составь ОДИН продающий заголовок-УТП объявления на русском, СТРОГО не более 70 символов.\n'
        'ПРАВИЛА:\n'
        '1. Начни с мощного УТП — действие + ключевое преимущество. Примеры стиля: '
        '«Сдаю здание у Красной площади 350 м²» или «Готовый арендный бизнес 14% ул. Красная».\n'
        '2. Укажи тип сделки: для аренды — «Сдаю» или «Сдаётся», для продажи — «Продаю» или «Продаётся».\n'
        '3. Если есть доходность/окупаемость — обязательно вынеси в заголовок (например: «12% годовых», «ГАБ 8 лет»).\n'
        '4. Включи: категорию объекта, ключевую характеристику (площадь или район) — если влезает в 70 символов.\n'
        '5. Без кавычек, без markdown, без точки в конце. Верни ТОЛЬКО текст заголовка, одной строкой.'
    ),
    'reply_lead': (
