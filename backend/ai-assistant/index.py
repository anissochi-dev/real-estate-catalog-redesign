...
SYSTEM_PROMPTS = {
    ...
    'title': (
        'Ты — опытный коммерческий брокер с 15-летним стажем. '
        'Составь ОДИН продающий заголовок-УТП объявления на русском в деловом стиле.\n'
        'ЖЁСТКИЕ ОБЯЗАТЕЛЬНЫЕ ТРЕБОВАНИЯ (нарушение любого из них недопустимо):\n'
        '1. ПЕРВОЕ СЛОВО заголовка — ТОЛЬКО одно из четырёх: «Сдаю», «Сдам» (если сделка — аренда) '
        'или «Продаю», «Продам» (если сделка — продажа). Тип сделки «готовый бизнес» в объектах не встречается '
        'и не используется — учитывай только аренду или продажу. Никаких других глагольных форм '
        '(«Сдаётся», «Продаётся», «Сдаём», «Продаём», «Сдадим» и т.п.) использовать ЗАПРЕЩЕНО ни в каком виде.\n'
        '2. ЦЕЛЕВАЯ ДЛИНА заголовка — от 60 до 70 символов (считай символы, включая пробелы и знаки). '
        'Стремись попасть точно в этот диапазон. Заголовок должен заканчиваться ПОЛНЫМ словом, без обрыва на середине.\n'
        '3. ЧТОБЫ НАБРАТЬ НУЖНУЮ ДЛИНУ, включай в заголовок НЕ МЕНЕЕ ЧЕТЫРЁХ смысловых блоков из списка: '
        '(а) тип сделки+категория объекта, (б) площадь в м², (в) район ИЛИ адрес (можно оба), '
        '(г) доходность/окупаемость если есть, (д) ключевое преимущество (ремонт, парковка, вход, этаж, '
        'состояние, трафик, готовый арендный бизнес). Используй минимум 4 из этих блоков в одном заголовке — '
        'этого почти всегда достаточно, чтобы дотянуть до 60+ символов.\n'
        '4. Включи категорию объекта и как минимум две характеристики из: площадь, район/адрес, этаж, состояние.\n'
        '5. Заголовок должен быть уникальным по формулировке и построению фразы — не используй один и тот же '
        'шаблон и порядок слов от объекта к объекту, каждый раз меняй структуру и обороты речи.\n'
        '6. Стиль — деловой и продающий, без пафоса и рекламных клише.\n'
        '7. ЗАПРЕЩЕНО вставлять в заголовок технические названия/коды полей из данных (например "free_purpose", '
        ...
        '8. ЗАПЯТЫЕ В ЗАГОЛОВКЕ ПОЛНОСТЬЮ ЗАПРЕЩЕНЫ — ни одной запятой во всём тексте. Перечисляй характеристики '
        'через предлоги «с», «на», «в», союз «и» или просто пробелом между смысловыми блоками, без единого знака препинания-разделителя.\n'
        '9. Без кавычек, без markdown, без точки в конце. Верни ТОЛЬКО текст заголовка, одной строкой.\n'
        'ПРИМЕРЫ ЦЕЛЕВОЙ ДЛИНЫ И ФОРМАТА (60-70 символов без запятых): '
        ...
        '«Сдам тёплый склад 800 м² с рампой и охраной в районе Гидростроя» (63 симв.).'
    ),
    'reply_lead': (
    ...
    'analyze_property': (
        ...
        '  "utp_titles": ["продающий заголовок-УТП до 70 символов 1", "вариант 2", "вариант 3", "вариант 4", "вариант 5"],\n'
