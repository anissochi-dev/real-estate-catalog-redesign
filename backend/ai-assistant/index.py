...
SYSTEM_PROMPTS = {
    'describe': (
        ...
        '- Все заполненные характеристики из карточки должны быть отражены в описании. '
        ...
        'бизнес-центры, торговые центры, образовательные учреждения, тренды и перспективы развития территории); '
        ...
        'Если пишешь блок: детализируй доход, расчётную доходность в % годовых, срок окупаемости, драйверы увеличения дохода.\n\n'
        ...
        'ВАЖНЫЕ ПРАВИЛА:\n'
        '- Заголовки блоков (там, где они предусмотрены) пиши ТОЧНО как указано выше, на отдельной строке, БЕЗ ДВОЕТОЧИЯ В КОНЦЕ '
        '(не «От собственника! Без комиссий и %!:», а именно как в образце), без markdown, без звёздочек * и без символа #.\n'
    ...
    'analyze_property': (
        ...
        '  "description": "Готовое продающее описание. ОБЯЗАТЕЛЬНО начинается: От собственника! Без комиссий и %. Далее строго по структуре: 1) Общие параметры и назначение 2) Локация и район (инфраструктура, трафик, тренды развития) 3) Характеристики объекта (площадь, этаж, отделка, вход, парковка — каждое в выгоду) 4) Коммуникации (только из данных карточки) 5) Финансовые перспективы и доходность (если есть данные — расчёт %, окупаемость; если нет — рыночный потенциал) 6) Условия (собственник, документы готовы, прямая продажа) 7) Призыв к действию (ограниченность предложения, торг уместен или только для инвесторов). Деловой уверенный стиль. Без markdown и символов * и #. Объём 2000-3500 символов."\n'
        '}'
    ),
    'search_leads': (
