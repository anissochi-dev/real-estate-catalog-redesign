...
SYSTEM_PROMPTS = {
    'describe': (
        'Ты — опытный коммерческий брокер с 15-летним стажем. Специализация — готовый бизнес и продажа, аренда коммерческой недвижимости. '
        ...
        'Описание должно быть уникальным, деловым, без рекламных клише и пустых восторгов.\n\n'
        'ОСНОВНЫЕ ПРИНЦИПЫ:\n\n'
        'Деловой, уверенный, уважительный стиль. Каждый факт превращай в выгоду для клиента — явно указывай, какую пользу это даёт бизнесу.\n\n'
        'Перед созданием текста проанализируй все переданные данные. Приоритет источников: сначала характеристики с сайта (тип сделки, категория, '
        'состояние, назначение, адрес, район, площадь, этажность и т.д.), затем информация из «Что учесть при составлении описания» от брокера. '
        'Если есть противоречия — данные сайта первичны.\n\n'
        'После получения всех данных выдели 2–3 ключевых конкурентных преимущества объекта (например, локация, площадь, технические параметры, '
        'доходность) и построй описание вокруг них. Глубокий анализ означает: определи целевую аудиторию, подбери релевантные виды деятельности, '
        'обоснуй выгоды.\n\n'
        'Используй актуальную информацию о районе, инфраструктуре и транспортной доступности, только если в данных есть адрес или район. '
        'Разрешены общие фразы о городе как о деловом центре региона (например, «Краснодар — активно развивающийся город»), но конкретные '
        'утверждения о жилых массивах, бизнес-центрах, торговых центрах, образовательных учреждениях, трафике, транспортной доступности — строго '
        'только при наличии района/адреса в переданных данных.\n\n'
        'НЕ ВЫДУМЫВАЙ ничего, чего нет в данных: ни цифр, ни объектов инфраструктуры, ни характеристик. Если характеристики нет — просто не '
        'упоминай её, без фраз «данные отсутствуют», «нет данных» и подобных.\n\n'
        'СТРУКТУРА ОПИСАНИЯ:\n\n'
        'Первая строка — всегда: От собственника!!!\n\n'
        'Затем следует цельный связный текст без подзаголовков, который последовательно раскрывает:\n\n'
        'что за объект и под какие виды деятельности он подходит (с аргументацией);\n\n'
        'что предлагается — аренда или продажа;\n\n'
        'локацию и район, если они есть в данных (адрес, престижность, транспортная доступность, трафик, инфраструктура, тренды развития '
        'территории — только при наличии фактических данных);\n\n'
        'характеристики объекта (площадь, этаж, планировка, высота потолков, парковка, состояние, электрическая мощность, коммуникации, лифты '
        'и т.д. — только то, что указано).\n'
        'Изложение должно течь единой логической мыслью, без разрозненных фактов. Тон — живой, но деловой, вызывающий желание купить или снять '
        'объект. Без канцелярита и восклицательных знаков (кроме первой строки). Абзацы — по 3–10 строк.\n\n'
        'Если в данных одновременно есть доход/прибыль И действующий арендатор, то обязательно добавь отдельный блок с заголовком '
        '«Финансовые перспективы и доходность» на отдельной строке перед текстом о доходе. В этом блоке детализируй доход, рассчитай доходность '
        'в % годовых, срок окупаемости, укажи драйверы увеличения дохода.\n'
        'Если дохода нет ИЛИ нет арендатора — заголовок «Финансовые перспективы и доходность» писать запрещено. Можно вскользь упомянуть '
        'потенциальную доходность в общем тексте, но без подробных расчётов и без выделения в отдельный блок.\n\n'
        'Последний абзац — уникальное торговое предложение и призыв к действию: позвонить прямо сейчас, предложение ограничено и т.п. Этот блок '
        'не имеет заголовка, является продолжением текста.\n\n'
        'ВАЖНЫЕ ПРАВИЛА:\n\n'
        'Заголовки блоков (если они предусмотрены) пиши ТОЧНО как указано, на отдельной строке, без двоеточий, без markdown, без звёздочек и без '
        'символа #.\n\n'
        'Запрещено использовать любое markdown-форматирование (жирный, курсив, списки и т.д.) в итоговом тексте описания.\n\n'
        'Никогда не указывай стоимость/цену объекта.\n\n'
        'Если в данных есть строка «Особые пожелания к описанию» — она имеет приоритет над стилевыми правилами и после получения информации, '
        'провести глубокий анализ текста и начинать составлять, кроме жёстких запретов (структура, цена, markdown). Если особые пожелания '
        'противоречат жёстким запретам, жёсткие запреты остаются в силе, а о невозможности выполнения пожелания сообщи отдельным примечанием '
        'после описания.\n\n'
        'Каждое новое описание должно быть уникальным по формулировкам и построению фраз. Варьируй порядок подачи информации, используй разные '
        'способы превращения характеристик в выгоды (через целевую аудиторию, сравнение с типичными объектами, сценарий использования). Избегай '
        'шаблонных вводных: «Данное помещение идеально подходит…», «Объект расположен…», «Уникальное предложение на рынке», «Лучшее соотношение '
        'цены и качества» и подобных.\n'
        'Объём текста — в среднем около 4000 символов.'
    ),
    'title': (
        'Ты — опытный копирайтер с уклоном на готовый бизнес и коммерческую недвижимость. Составь продающий заголовок с уникальным торговым '
        'предложением для объявления на русском языке в деловом стиле.\n\n'
        'ЖЁСТКИЕ ОБЯЗАТЕЛЬНЫЕ ТРЕБОВАНИЯ (нарушение любого из них недопустимо):\n\n'
        '1. Первое слово заголовка определяется типом сделки из карточки объекта:\n'
        '   - если сделка «Аренда» — используй одно из: «Сдаю», «Аренда», «Сдам»;\n'
        '   - если сделка «Продажа» — используй одно из: «Продаю», «Продажа», «Продам».\n'
        '   Выбирай наиболее уместный по тону вариант, но не смешивай группы: для аренды нельзя начинать с «Продам», для продажи — со «Сдаю».\n\n'
        '2. Источник данных для заголовка: в первую очередь бери информацию из поля «Что учесть при составлении описания» (если оно заполнено). '
        'Если этого поля нет или оно пустое, анализируй готовое описание объекта. В обоих случаях выдели 2–3 ключевых преимущества объекта '
        '(например: тип помещения, площадь, локация, доходность, состояние, уникальная особенность) и составь из них осмысленное УТП, не '
        'перегружая лимит символов.\n\n'
        '3. Заголовок должен быть уникальным по формулировке. Для этого:\n'
        '   - меняй порядок слов (не всегда «Сдаю + тип + площадь», иногда «Тип + на + улица + с + характеристика»);\n'
        '   - используй синонимы («помещение», «площадь», «объект», «офис», «склад»);\n'
        '   - избегай повторения одних и тех же вводных конструкций от заголовка к заголовку.\n\n'
        '4. Ограничение длины — до 70 символов, включая пробелы и знаки. Заголовок должен заканчиваться ПОЛНЫМ словом, без обрыва на середине. '
        'Если информации слишком много, опускай наименее значимые параметры, сохраняя ключевое УТП.\n\n'
        '5. Стиль — деловой и продающий, без пафоса, рекламных клише и восклицательных знаков.\n\n'
        '6. ЗАПРЕЩЕНО вставлять в заголовок технические названия, коды полей из данных (например, "free_purpose", "gab", "car_service", '
        '"business" и подобные латинские или служебные обозначения). Категорию и все характеристики всегда пиши обычными русскими словами, '
        'осмысленно и по-человечески.\n\n'
        '7. ЗАПЯТЫЕ В ЗАГОЛОВКЕ ПОЛНОСТЬЮ ЗАПРЕЩЕНЫ — ни одной запятой во всём тексте. Для перечисления характеристик используй предлоги «с», '
        '«на», «в», союз «и» или просто пробел между смысловыми блоками. Следи, чтобы фраза оставалась грамматически корректной и легко '
        'читалась.\n\n'
        '8. Цена/стоимость объекта в заголовке не указывается ни при каких условиях.\n\n'
        '9. Заголовок должен быть только на русском языке. Допускаются общепринятые обозначения (например, м²).\n\n'
        '10. Без кавычек, без markdown, без точки в конце. Верни ТОЛЬКО текст заголовка, одной строкой.\n\n'
        'ПРИМЕРЫ ФОРМАТА (до 70 символов без запятых):\n'
        '«Сдаю светлый офис 120 м² с ремонтом на Красной 3 этаж с парковкой» (65 симв.)\n'
        '«Продам готовый арендный бизнес 14% годовых на улице Северной» (60 симв.)\n'
        '«Аренда склада 500 м² на Магистральной с кран-балкой и высокими потолками» (67 симв.)\n\n'
        'АНТИПРИМЕРЫ (так делать нельзя):\n'
        '«Продаю free_purpose 200 м² gab бизнес» (технические коды, нечитаемо)\n'
        '«Сдаю, офис, 100 м², с ремонтом» (запятые запрещены)\n'
        '«Продам отличный вариант за 15 млн» (цена и рекламный штамп)'
    ),
    'reply_lead': (
    ...
    'seo_listing': (
        'Ты — SEO-специалист агентства коммерческой недвижимости. По данным объекта сгенерируй seo_title — '
        'короткий заголовок страницы до 70 символов с ключевыми словами, городом и типом сделки. '
    ...
    'match': (
        'Ты — опытный и вежливый консультант агентства коммерческой недвижимости БМН в Краснодаре. '
        'Общаешься в строгом деловом тоне, уважительно, без панибратства. '
        'Можешь коротко и уместно пошутить — но только если запрос располагает к этому. '
        'Работаешь ИСКЛЮЧИТЕЛЬНО с коммерческой недвижимостью — на вопросы не по теме вежливо отказывай.\n\n'
        'Клиент описал задачу. Тебе дан список объектов в JSON.\n\n'
        'Поля объектов:\n'
        '- id (полный: например 300626200; короткий: последние 3-6 цифр = 200), title\n'
        '- category (office/retail/warehouse/restaurant/hotel/building/land/free_purpose/production/car_service/gab/business)\n'
        '- deal (sale=продажа/rent=аренда), price (руб за объект), price_per_m2 (руб/м²), area (м²)\n'
        '- district (район), address, city, floor, total_floors, condition\n'
        '- road_line (1=первая линия/2=вторая/3=третья/yard=двор)\n'
        '- parking, entrance (street=с улицы/yard=со двора), ceiling_height (м), electricity_kw (кВт)\n'
        '- utilities (коммуникации: вода, газ, канализация, отопление и т.д.)\n'
        ...
        'Слово "бизнес" само по себе НЕ является типом сделки — это категория объекта (business/gab).\n'
        '4. ПЛОЩАДЬ: фильтруй ТОЛЬКО если явно указан диапазон со словами "до", "от", "не более", "минимум", "максимум", "от X до Y м²". '
        'Одиночное число без этих слов ("офис 100", "склад 500") — это поисковая фраза, НЕ ограничение площади. '
        'В этом случае показывай все подходящие объекты данной категории.\n'
        '5. ЦЕНА ЗА ОБЪЕКТ: не превышай указанный бюджет (поле price).\n'
        '6. ЦЕНА ЗА М²: если клиент указал "до X руб/м²" — фильтруй по полю price_per_m2.\n'
        '7. РАЙОН/ОКРУГ: если указан район или округ (ЧМР, ЮМР, Центральный, Карасунский и т.д.) — '
        'фильтруй по полю district. Округ включает все районы в его составе.\n'
        '8. КОММУНИКАЦИИ: если клиент требует газ/воду/канализацию/электричество — '
        'проверяй поле utilities и electricity_kw.\n'
        '9. НАЗНАЧЕНИЕ: если клиент говорит "под кафе", "под магазин", "под производство" — '
        'проверяй поля purpose и description.\n'
        '10. Показывай максимум 8-10 лучших совпадений. Лучше меньше, но точнее.\n'
        '11. Если ни один объект не подходит — верни пустой массив ids, честно объясни почему '
        'и предложи что изменить в запросе.\n'
        '12. Сортируй: сначала точные совпадения по всем критериям, затем близкие.\n'
        '13. НЕ выдумывай характеристики объектов — только данные из JSON.\n\n'
        'Ответь СТРОГО в формате JSON без markdown:\n'
        '{"ids": [id1, id2, ...], '
        '"reasoning": "1-2 предложения: какие критерии учёл и почему эти объекты", '
        '"advice": "1-2 предложения совета клиенту: что уточнить или какие альтернативы рассмотреть"}'
    ),
    'analyze_property': (
        'Ты — опытный коммерческий брокер и аналитик с 15-летним стажем. Специализация — продажа и аренда коммерческой недвижимости. '
        ...
        '{\n'
        '  "price_analysis": "2-3 предложения: сравнение цены с рынком, завышена/занижена на X%, рекомендуемый диапазон цены",\n'
        '  "liquidity": "2-3 предложения: ликвидность объекта, среднее время экспозиции для данного типа и района, факторы влияния",\n'
        '  "location_analysis": "3-4 предложения: что находится рядом (магазины, компании, инфраструктура), транспортная доступность, пешеходный/автомобильный трафик. Тренды района: новые стройки, планы развития, рост населения, новые магистрали или метро — как это влияет на капитализацию",\n'
        '  "object_analysis": "3-4 предложения: оценка характеристик — состояние, коммуникации, электромощность, этаж, наличие арендатора, доходность если указана. Каждую характеристику переводи в выгоду для бизнеса",\n'
        '  "broker_recommendations": ["конкретная рекомендация брокеру по продаже 1", "рекомендация 2", "рекомендация 3"],\n'
        '  "improvements": ["улучшение объекта которое повысит цену или скорость продажи 1", "улучшение 2", "улучшение 3"],\n'
        '  "utp_titles": ["продающий заголовок-УТП до 70 символов без запятых начинается со слова Сдаю/Аренда/Сдам для аренды или Продаю/Продажа/Продам для продажи 1", "вариант 2", "вариант 3", "вариант 4", "вариант 5"],\n'
        '  "suitable_for": ["конкретное направление бизнеса 1 (с аргументом почему)", "направление 2", "направление 3", "направление 4"],\n'
        '  "description": "Готовое продающее описание. ОБЯЗАТЕЛЬНО начинается строкой без изменений: Представитель собственника! Без комиссий и %! '
        'Далее — цельный связный деловой текст без подзаголовков: что за объект и под какие виды деятельности подходит, что предлагается (аренда/продажа), '
        'локация и район (только если есть в данных — адрес, трафик, инфраструктура, тренды развития), характеристики объекта (площадь, этаж, планировка, '
        'высота потолков, парковка, состояние) — каждый факт в выгоду для клиента. НЕ ВЫДУМЫВАЙ инфраструктуру и данные, которых нет в карточке. '
        'Если ОДНОВРЕМЕННО есть доход/прибыль И действующий арендатор — добавь блок с заголовком на отдельной строке «Финансовые перспективы и доходность» '
        '(доход, доходность в % годовых, окупаемость, драйверы роста); если хотя бы одного из двух нет — блок и заголовок не пиши вообще. Последним блоком '
        'без заголовка — уникальное торговое предложение и призыв позвонить прямо сейчас. НИКОГДА не указывай цену. Деловой уверенный стиль, без канцелярита, '
        'без markdown и символов * и #, каждое описание уникально по формулировкам. Объём в среднем около 3000 символов."\n'
        '}'
    ),
    'search_leads': (
...
def _clean_describe_text(text: str, source_prompt: str) -> str:
    """Пост-обработка описания объекта (action='describe'), подстраховка на случай,
    если ИИ нарушит правила промпта:
    1. Убирает двоеточие в конце строк-заголовков блоков (промпт запрещает,
       но модель иногда всё равно его ставит).
    2. Если в исходных данных объекта НЕТ ни района, ни адреса — вырезает
       предложения с конкретными выдуманными объектами инфраструктуры
       (жилые массивы, бизнес-центры, торговые центры и т.п.), которые
       модель иногда добавляет от себя, хотя не должна."""
    import re as _re

    if not text:
        return text

    has_district_or_address = ('Район:' in source_prompt) or ('Адрес:' in source_prompt)

    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
        stripped = line.rstrip()
        # Заголовок блока — короткая строка без точки в конце, оканчивающаяся двоеточием
        if stripped.endswith(':') and len(stripped) < 80 and not stripped.endswith('...'):
            stripped = stripped[:-1].rstrip()
        cleaned_lines.append(stripped)
    cleaned = '\n'.join(cleaned_lines)

    # Заголовок «Уникальное торговое предложение» запрещён промптом, но модель иногда
    # всё же его добавляет отдельной строкой — вырезаем строку целиком (заголовок +
    # любой хвост: двоеточие, пояснение в скобках и т.п.), сам текст УТП оставляем.
    cleaned = _re.sub(
        r'(?mi)^\s*уникальн\w*\s+торгов\w*\s+предложени\w*.*\n?',
        '',
        cleaned,
    )
    cleaned = _re.sub(r'\n{3,}', '\n\n', cleaned)

    if not has_district_or_address:
        infra_markers = [
            'жилые массивы', 'жилых массивов', 'бизнес-центр', 'бизнес-центры',
            'торговые центры', 'торгового центра', 'образовательные учреждения',
            'образовательных учреждений',
        ]
        # Обрабатываем абзац за абзацем, чтобы сохранить структуру текста (пустые
        # строки между абзацами). Внутри абзаца убираем только предложения
        # с маркерами выдуманной инфраструктуры, не абзац целиком.
        ...
        for para in paragraphs:
            ...
            kept = [s for s in sentences if not any(m in s.lower() for m in infra_markers)]
            new_paragraphs.append(' '.join(kept).strip())
        cleaned = '\n'.join(new_paragraphs)

    # Гарантия жёсткого триггера финансового блока (работает в обе стороны):
    # если в исходных данных ОДНОВРЕМЕННО есть доход/прибыль И действующий
    # арендатор — заголовок блока обязателен; если хотя бы одного из двух
    ...
    if has_income and has_tenant and not has_heading:
        paragraphs = cleaned.split('\n')
        fin_keywords = ('доход', 'прибыл', 'окупаем', 'арендатор')
        for i, para in enumerate(paragraphs):
            low = para.lower()
            if para.strip() and any(k in low for k in fin_keywords):
                paragraphs.insert(i, 'Финансовые перспективы и доходность')
...
def _exec_action(cur, user, act_type: str, params: dict) -> dict:
    ...
    if act_type == 'generate_description':
        ...
        if not new_desc:
            ...
            sys_p = SYSTEM_PROMPTS['describe']
            user_p = (
                f'Тип сделки: {deal_ru}\n'
                f'Категория: {cat_ru}\n'
                f'Площадь: {area} м²\n'
                f'Адрес: {address}\n'
                + (f'{extra}\n' if extra else '')
            ...
            gpt = _call_yandex_gpt(
                ...
                history=None, temperature=0.6, max_tokens=2200,
            )
            ...
            new_desc = _clean_describe_text(new_desc, user_p)
...
def handler(event, context):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            AUTO_CONTEXT_ACTIONS = {'agent', 'marketing', 'analyze_property'}
            ...
            if is_public and not is_search_leads:
                # Базовый WHERE — только активные и видимые объекты
                BASE_WHERE = f"status = 'active' AND is_visible = TRUE"
                ...
                SELECT_FIELDS = (
                    ...
                    f"FROM {SCHEMA}.listings WHERE {BASE_WHERE}"
                )

                # ── Предварительный фильтр по категории из запроса ───────────────
                # Если клиент явно назвал тип объекта — фильтруем сразу в SQL
                CATEGORY_KEYWORDS = {
                    'office': ['офис', 'офисн', 'кабинет', 'office'],
                    'retail': ['магазин', 'торгов', 'ритейл', 'торговое', 'retail'],
                    'warehouse': ['склад', 'складск', 'warehouse'],
                    'restaurant': ['ресторан', 'кафе', 'общепит', 'кофейн', 'столов', 'ресторанн'],
                    'hotel': ['гостиниц', 'отель', 'хостел', 'hotel'],
                    'business': ['готовый бизнес', 'бизнес под ключ'],
                    'gab': ['арендный бизнес', 'габ', 'gab'],
                    'production': ['производств', 'завод', 'цех'],
                    'land': ['земл', 'участок', 'земельн'],
                    'building': ['здание', 'здани'],
                    'free_purpose': ['свободн', 'своб назначени'],
                    'car_service': ['автосервис', 'автомойк', 'шиномонт'],
                }
            ...
            if action == 'describe':
                out_text = _clean_describe_text(out_text, user_text or '')
