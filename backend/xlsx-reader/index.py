...
def handler(event: dict, context) -> dict:
    ...
    # Импорт рыночных объявлений (ЦИАН/Авито/другой источник) в market_listings
    if body.get('action') == 'import_market':
        file_url = body.get('file_url', '')
        source   = body.get('source', 'xlsx')
        preview  = body.get('preview', False)
        replace  = body.get('replace', False)
        if not file_url:
            return {'statusCode': 400, 'headers': {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'}, 'body': json.dumps({'error': 'Укажите file_url'}, ensure_ascii=False)}
        return _import_market_xlsx(file_url, source, preview, replace)
