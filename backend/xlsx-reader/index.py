...
def _run_import_job(job_id: int):
    ...
    try:
        ...
        # Агрегируем market_listings → price_market_snapshots в фоне (не блокируем таймаут)
        if inserted > 0:
            ...
            def _bg_agg():
                try:
                    ...
                    try:
                        _r = _aml(_c.cursor(), _c)
                        print(f'[xlsx-reader] bg aggregate done: {_r}')
                    finally:
                        _c.close()
                except Exception as _e:
                    print(f'[xlsx-reader] bg aggregate error: {_e}')
...
def handler(event: dict, context) -> dict:
    ...
    if body.get('action') == 'import_market_continue':
        ...
        rows_inserted = job['rows_inserted'] or 0
        rows_updated  = job['rows_updated'] or 0
        rows_skipped  = job['rows_skipped'] or 0
        cat_counts    = dict(job['category_breakdown'] or {})
