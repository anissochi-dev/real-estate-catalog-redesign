  ...
  useEffect(() => {
    ...
    if (!clustererRef.current) {
      clustererRef.current = new window.ymaps.Clusterer({
        ...
        // Клик по кластеру приближает карту, разбивая его на более мелкие группы/метки —
        // штатное поведение, нужное при больших кластерах (десятки объектов). Раньше зум
        // прижимал объекты вплотную к краям экрана без отступов — теперь задаём zoomMargin,
        // встроенный отступ от границ видимой области карты, чтобы метки не оказывались
        // впритык к краю или под нашими плашками поверх карты ("Краснодар", "Скрыть").
        // Если объекты в кластере физически совпадают по координатам (дальше приближать
        // некуда) — Яндекс.Карты сами откроют балун-карусель (настроен ниже) вместо зума.
