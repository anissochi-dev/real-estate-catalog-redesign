...
6. Сформулируй УТП для названия объекта (5–7 вариантов). Каждый вариант — до 70 символов, без запятых, первое слово строго «Сдаю»/«Аренда»/«Сдам» для аренды или «Продаю»/«Продажа»/«Продам» для продажи.
7. Напиши продающее описание объекта по шаблону:
   - Начало: «Представитель собственника! Без комиссий и %!» — строго без изменений, первой строкой.
   - Далее — цельный связный деловой текст без подзаголовков: что за объект, под какие виды деятельности подходит, локация и район (только если есть в данных), характеристики объекта — каждый факт в выгоду для клиента. Не выдумывай данные, которых нет.
   - Затем, при необходимости — рекомендации по улучшению.
   - В конце — перспективы объекта (для кого подойдёт, какие направления бизнеса) и призыв позвонить прямо сейчас.
   - Никогда не указывай цену/стоимость.
   - Объём: в среднем около 3000 символов. Стиль: деловой, уверенный, но живой, без канцелярита и пустых восторгов. Пиши сплошным текстом с абзацами, без markdown.
