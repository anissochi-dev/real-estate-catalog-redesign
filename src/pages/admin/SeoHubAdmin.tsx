...
import { SeoStatus, seoUrl, seoHeaders } from './seo/seoTypes';

type HubTab = 'site' | 'listings' | 'pages' | 'technical';
...
// ── Счётчики аналитики (вынесено из SeoTab для переиспользования) ───────────
type CheckStatus = 'idle' | 'checking' | 'ok' | 'err';
interface CheckState { status: CheckStatus; message: string }
const IDLE_STATE: CheckState = { status: 'idle', message: '' };
...
// ── Вкладка «Технические» (robots, sitemap + счётчики из SeoTab) ─────────────
function TechnicalTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 sticky bottom-4 bg-white p-3 rounded-xl shadow z-20">
        <button onClick={save} className="btn-blue text-white px-6 py-3 rounded-xl font-semibold">Сохранить счётчики</button>
        {saved && <span className="text-emerald-600 text-sm font-semibold">Сохранено ✓</span>}
      </div>
    </div>
  );
